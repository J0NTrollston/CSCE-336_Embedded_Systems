...
#include <avr/io.h>
#include "motors.h"
volatile uint16_t compareRegA = 175;
volatile uint16_t compareRegB = 180;
 ...
 **/
void driveForward(int numOfQuarterseconds){
...
**/
void driveBackward(int numOfQuarterseconds){
  OCR0A = compareRegA;
  OCR0B = compareRegB;
  PORTC_Reg &= ~(1<<In1);
  PORTB_Reg &= ~(1<<In4);
 ...
 * compareRegA will be pwm pulse width where 160 is min, 255 is max
**/
void driveLeft(int numOfQuarterseconds){
  OCR0A = compareRegA;
  OCR0B = compareRegB;
  PORTC_Reg &= ~(1<<In1);
  PORTB_Reg |= (1<<In4);
 ...
 * compareRegA will be pwm pulse width where 160 is min, 255 is max
**/
void driveRight(int numOfQuarterseconds){
  OCR0A = compareRegA;
  OCR0B = compareRegB;
  PORTC_Reg |= (1<<In1);
  PORTB_Reg &= ~(1<<In4);
...
/**
 * increases speed of the motors by 5, there is no limit. 
 * Range of 2^8=1 can be overshot. Be careful
**/
void increaseSpeed(){
  compareRegA +=5;
  compareRegA +=5;
...
/**
 * decreases the speed of the motors by 5, there is no limit.
 * value can decrease below threshold average voltage
**/
void decreaseSpeed(){
  compareRegA -=5;
  compareRegB -=5;
